...
from handlers.api_handlers import search, json_text, hotel_lowprice, photo
...
def lowprice_search(city):  # функция для поиска id города
...
def result(choice_photo, mod, message) -> None:
    if choice_photo == 'да':
        result_yes(mod, message)
    else:
        result_no(mod, message)
...
def lowprice_get_city(message: Message) -> None:  # Вводим город
...
def get_choice_photo(message: Message) -> None:  # Нужно ли загружать фотографии отелей?
...
def date_checkin(message: Message) -> None:  # Вводим дату заселения
...
def date_checkout(message: Message) -> None:  # Вводим дату выселения
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data_low:  # Дата выселения
        data_low['checkout'] = message.text
        if not message.text.isalpha():
            lowprice_text = hotel_lowprice.search(data_low['id_city'], data_low['checkin'], data_low['checkout'])
            mod_lowprice = json_text.json_mod(lowprice_text, 'text2.json')
            result(data_low['choice_photo'], mod_lowprice, message)
        else:
...
#
#     else:
#         bot.send_message(message.from_user.id, 'Кол-во фотографий может быть только числом')
#
#         with bot.retrieve_data(message.from_user.id, message.chat.id) as data_low:
#             hotel_lowprice.search(data_low['id_city'], )
#
#

# @bot.message_handler(state=UserInfoState.result)
# def get_info(message: Message) -> None:
#     with bot.retrieve_data(message.from_user.id, message.chat.id) as data_low:
#         sz = search.search(data_low['city'])
#         bot.send_message(message.from_user.id, data_low['city'])


# bot.set_state(message.from_user.id, UserInfoState.result, message.chat.id)
# with bot.retrieve_data(message.from_user.id, message.chat.id) as data_low:
#     sz = search.search(data_low['city'])
#     bot.send_message(message.from_user.id, data_low['city'])
